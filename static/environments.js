(() => {
    ...
    function modal() {
        ...
        node.querySelector(".environments-create").onsubmit = async (event) => {
            ...
            try {
                await request("/api/environments", { method: "POST", body: JSON.stringify({ branch: input.value.trim() }) });
                input.value = "";
                await load();
            } catch (error) {
    ...
    function render() {
        ...
        list.innerHTML = state.environments.length ? state.environments.map((env) => {
            const action = env.status === "RUNNING" ? "stop" : "start";
            const label = env.status === "RUNNING" ? "Стоп" : "Старт";
            return [
                '<article class="environment-card">',
                '<div class="environment-card-main"><strong>', esc(env.branch_name), '</strong>',
                '<code>', esc(String(env.commit_sha).slice(0, 12)), '</code>',
                '<span class="environment-status environment-status-', esc(String(env.status).toLowerCase()), '">', esc(env.status), '</span></div>',
