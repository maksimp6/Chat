(function () {
  ...
  function renderStatus(container, providers) {
    ...
    (providers || []).forEach(function (item) {
      ...
      meta.textContent =
        (fp ? "fp: " + fp.slice(0, 12) : "fingerprint: —") + (exp ? " • expires: " + exp : "");
  ...
  function build() {
    ...
    box.appendChild(
      makeField("provider-yandex-key", "Yandex Cloud API key", "", "Yandex API key", "password"),
    );
    box.appendChild(
      makeField(
        "provider-yandex-project",
        "Yandex Cloud Project ID",
        "Обязателен вместе с Yandex API key.",
        "например: b1g1fekh2198nuan1tnh",
        "text",
      ),
    );
    box.appendChild(
    ...
    save.textContent = "Подключить";
    save.className = "alice-btn btn-primary";
    save.addEventListener("click", async function () {
      ...
      output.textContent = "";
      if (y && !project) {
        setFieldError("provider-yandex-project", "Project ID обязателен вместе с Yandex API key.");
        return;
      }
      if (!y && project) {
    ...
    output.className = "provider-credentials-status";
