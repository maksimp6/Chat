(function () {
  ...
  function openPartnerRelationsModal() {
    ...
    var notice = h(
      "div",
      { style: "font-size:12px;opacity:.72;margin:8px 0 12px;" },
    ...
    [name, organization, email, phone, create].forEach(function (n) {
      form.appendChild(n);
    });
    ...
    function load() {
      return window.AliceDispatcher.request("/api/partners", {
          ...
          partners.forEach(function (partner) {
            ...
            row.appendChild(
              h(
                "span",
                {},
                [partner.organization, partner.status, partner.email].filter(Boolean).join(" · "),
              ),
            );
            row.onclick = function () {
              window.AliceDispatcher.request("/api/partners/" + encodeURIComponent(partner.id), {
                headers: { Accept: "application/json" },
              })
                .then(function (r) {
                  return r.json().then(function (data) {
    ...
    function showDetails(partner) {
      ...
      detail = h("div", {
        id: "partner-relations-detail",
        style: "margin-top:14px;padding-top:14px;border-top:1px solid rgba(127,127,127,.25);",
      });
      detail.appendChild(
        h("h4", { style: "margin:0 0 8px;" }, partner.name + " · " + partner.status),
      );

      var status = h("select", { "aria-label": "Статус партнёра" });
      ["lead", "active", "negotiating", "paused", "completed", "archived"].forEach(
        function (value) {
      ...
      statusButton.onclick = function () {
        window.AliceDispatcher.request(
          "/api/partners/" + encodeURIComponent(partner.id) + "/status",
          {
            method: "POST",
      ...
      detail.appendChild(
        h(
          ...
          "Контакты (" + (partner.contacts || []).length + ")",
        ),
      );
      ...
      detail.appendChild(
        h(
          "h5",
          { style: "margin:14px 0 6px;" },
          "Переписка (" + (partner.messages || []).length + ")",
        ),
      );
        ...
        detail.appendChild(
          h(
            ...
            "[" +
              ...
              "] " +
              message.subject +
              (message.body ? ": " + message.body.slice(0, 160) : ""),
          ),
        );
      });
      ...
      detail.appendChild(
        h(
          "h5",
          { style: "margin:14px 0 6px;" },
          "Follow-up (" + (partner.followups || []).length + ")",
        ),
      );
      (partner.followups || []).forEach(function (followup) {
        var line = h("div", {
          style: "display:flex;gap:8px;align-items:center;font-size:12px;padding:6px 0;",
        });
        ...
        if (followup.status === "open") {
          var done = h("button", { type: "button" }, "Готово");
            ...
            window.AliceDispatcher.request(
              "/api/partners/followups/" + encodeURIComponent(followup.id) + "/complete",
              {
                method: "POST",
                headers: { "Content-Type": "application/json" },
              ...
              .then(function (r) {
                return r.json().then(function (data) {
                  if (!r.ok) throw new Error(data.error || "HTTP " + r.status);
                  load().then(function () {
                    showDetails(data.followup ? partner : partner);
                  });
                });
              })
              .catch(function (err) {
