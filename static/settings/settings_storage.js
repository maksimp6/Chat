(function () {
  ...
  function convKey(convId) {
    return STORAGE_KEY + "_conv_" + convId;
  }

  // Синхронная загрузка (из кэша или localStorage для обратной совместимости)
  function load(convId) {
    convId =
      convId || (typeof currentConvId !== "undefined" && currentConvId ? currentConvId : null);
  ...
  function save(settings, convId) {
    ...
    if (convId) {
      ...
      window.AliceDispatcher.request("/api/conversations/" + convId + "/settings", {
        ...
        console.warn("[SETTINGS] Failed to save on server:", e);
      });
    } else {
  ...
  window.loadServerConvSettings = function (convId) {
    ...
    return window.AliceDispatcher.request("/api/conversations/" + convId + "/settings", null, {
      timeoutMs: 5000,
    })
      .then(function (r) {
        return r.json();
  ...
  window.getSettings = function (convId) {
    return load(convId);
  };
  window.saveSettings = function (settings, convId) {
    save(settings, convId);
  };
  ...
  window.clearConversationSettings = function (convId) {
    if (convId) localStorage.removeItem(convKey(convId));
  };
