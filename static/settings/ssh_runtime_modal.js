  ...
  window.openSshRuntimeModal = function () {
    ...
    var tabSsh = [
      ...
      UI.gap2(
        ...
        "<div>" +
          UI.lbl("Макс. вывод (байт)") +
          UI.inp(
            "set-ssh-max-output",
            "number",
            1048576,
            ' min="4096" max="10485760" step="4096"',
          ) +
          "</div>",
      ),
      UI.gap2(
      ...
      '<div style="margin-top:10px;">' +
        UI.lbl("Общий known_hosts (серверный путь)") +
        UI.inp("set-ssh-known-hosts", "text", "", ' placeholder="/srv/alice/ssh/known_hosts"') +
        "</div>",
      '<div style="margin-top:10px;">' +
        UI.lbl("Command allowlist (regex, один шаблон на строку)") +
        UI.ta(
          "set-ssh-command-allowlist",
          "",
          "height:80px;font-family:monospace;font-size:11px;",
        ) +
        "</div>",
      UI.chk(
        "set-ssh-allow-privileged",
        false,
        "Разрешать privileged-команды (sudo/su/doas/pkexec)",
        "",
        "Опасные операции остаются под approval gate.",
      ),
      ...
      '<div style="margin-top:10px;">' +
        UI.lbl("Named targets (JSON)") +
        UI.ta("set-ssh-targets", "", "height:180px;font-family:monospace;font-size:11px;") +
        "</div>",
      '<div style="display:flex;gap:8px;align-items:end;margin-top:10px;">',
      '<div style="flex:1;">' +
        UI.lbl("Проверить target") +
        UI.inp("set-ssh-test-target", "text", "", ' placeholder="preview"') +
        "</div>",
    ...
    function populateSshSettings(data) {
      ...
      document.getElementById("set-ssh-command-allowlist").value = (
        ssh.command_allowlist || []
      ).join("\n");
      document.getElementById("set-ssh-allow-privileged").checked =
        ssh.allow_privileged_operations === true;
      document.getElementById("set-ssh-approval-write").checked =
        ssh.approval_required_for_write !== false;
      document.getElementById("set-ssh-approval-privileged").checked =
        ssh.approval_required_for_privileged !== false;
      ...
      if (sshTestTargetEl && targetNames.length && !sshTestTargetEl.value)
        sshTestTargetEl.value = targetNames[0];
      var last = ssh.last_test;
      if (last) {
        setSshStatus(
          "Последняя проверка: " +
            (last.success ? "успешна" : "ошибка") +
    ...
    document.getElementById("set-ssh-save-btn").addEventListener("click", function () {
      ...
      var payload = {
        ...
        max_output_bytes:
          parseInt(document.getElementById("set-ssh-max-output").value, 10) || 1048576,
        known_hosts: document.getElementById("set-ssh-known-hosts").value.trim() || null,
        command_allowlist: document
          .getElementById("set-ssh-command-allowlist")
          .value.split(/\r?\n/)
          .map(function (v) {
            return v.trim();
          })
        ...
        approval_required_for_privileged: document.getElementById("set-ssh-approval-privileged")
          .checked,
        targets: targets,
      };
