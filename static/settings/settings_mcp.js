(function () {
  ...
  window.fetchMcpServers = function () {
    ...
    mcpFetchPromise = window.AliceDispatcher.request("/api/mcp-servers")
      .then(function (r) {
        if (!r.ok) {
          console.warn("[MCP] Server returned " + r.status);
          return { data: [] };
        }
        return r.json();
      })
  ...
  window.getMcpServersCache = function () {
    return mcpServersCache;
  };
  ...
  function buildMcpTool(srv, approvalOverride) {
    ...
    if (srv.connector_id) {
      mcp.connector_id = srv.connector_id;
    } else if (srv.server_url) {
      mcp.server_url = srv.server_url;
    }
    ...
    if (srv.headers) {
      try {
        mcp.headers = JSON.parse(srv.headers);
      } catch (e) {}
    }
  ...
  window.openMcpManagerModal = function (onClose) {
    ...
    function renderList(servers) {
      ...
      if (!servers || servers.length === 0) {
        list.innerHTML =
          '<div style="padding:20px;text-align:center;color:var(--m-muted,#999);">Нет MCP серверов. Добавьте первый.</div>';
        return;
      }
      list.innerHTML = servers
        .map(function (srv) {
          ...
          return (
            '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px;border:1px solid var(--m-section-border,#eee);border-radius:6px;margin-bottom:8px;background:var(--m-card,#fff);">' +
            '<div><div style="font-weight:bold;font-size:14px;color:var(--m-text,#222);">' +
            UI.escapeHtml(srv.name) +
            "</div>" +
            '<div style="font-size:12px;color:var(--m-muted,#666);">' +
            UI.escapeHtml(connInfo) +
            "</div></div>" +
            '<div style="display:flex;gap:6px;">' +
            '<button class="alice-btn mcp-edit-btn settings-contract-btn" data-id="' +
            UI.escapeHtml(srv.id) +
            '">Изменить</button>' +
            '<button class="alice-btn mcp-del-btn settings-contract-btn" data-id="' +
            UI.escapeHtml(srv.id) +
            '">Удалить</button>' +
            "</div></div>"
          );
      ...
      list.querySelectorAll(".mcp-edit-btn").forEach(function (btn) {
        btn.addEventListener("click", function () {
          ...
          var srv = servers.find(function (s) {
            return s.id === sid;
          });
    ...
    function refreshList() {
      window.fetchMcpServers().then(renderList);
    }
    ...
    function openMcpEditor(srv) {
      ...
      var CONNECTOR_OPTS =
        '<option value="">(нет — использовать server_url)</option>' +
      ...
      e.style.cssText =
        "border:1px solid var(--m-accent,#4a90d9);border-radius:8px;padding:16px;margin-top:12px;background:var(--m-editor,#f8f9fa);";

      e.innerHTML =
        '<h3 style="margin:0 0 10px;font-size:15px;color:var(--m-text,#222);">' +
        (isNew ? "Новый MCP сервер" : "Редактирование") +
        "</h3>" +
        '<div style="margin-bottom:8px;">' +
        UI.lbl("Название") +
        UI.inp("mcp-ed-name", "text", srv.name || "", ' style="width:100%;"') +
        "</div>" +
        '<div style="margin-bottom:8px;">' +
        UI.lbl("Connector ID") +
        UI.sel("mcp-ed-connector", connSelected(srv.connector_id || "")) +
        "</div>" +
        '<div style="margin-bottom:8px;">' +
        UI.lbl("Server URL") +
        UI.inp(
          ...
          "text",
          srv.server_url || "",
          ' style="width:100%;" placeholder="https://mcp.example.com/sse"',
        ) +
        "</div>" +
        UI.gap2(
          "<div>" +
            UI.lbl("Server Label") +
            UI.inp("mcp-ed-label", "text", srv.server_label || "", ' style="width:100%;"') +
            "</div>",
          "<div>" +
            UI.lbl("Authorization") +
            UI.inp("mcp-ed-auth", "password", srv.authorization || "", ' style="width:100%;"') +
            "</div>",
        ) +
        '<div style="margin-bottom:8px;">' +
        UI.lbl("Description") +
        UI.inp("mcp-ed-desc", "text", srv.server_description || "", ' style="width:100%;"') +
        "</div>" +
        '<div style="margin-bottom:8px;">' +
        UI.lbl("Headers (JSON)") +
        '<textarea id="mcp-ed-headers" style="width:100%;min-height:40px;font-family:monospace;font-size:11px;padding:6px;border:1px solid var(--m-border,#ddd);border-radius:4px;background:var(--m-input-bg,#f5f5f5);color:var(--m-input-text,#222);box-sizing:border-box;" placeholder=\'{"X-Key":"val"}\'>' +
        UI.escapeHtml(srv.headers || "") +
        "</textarea></div>" +
        '<div style="margin-bottom:8px;">' +
        UI.lbl("Allowed Tools") +
        UI.inp("mcp-ed-allowed", "text", srv.allowed_tools || "", ' style="width:100%;"') +
        "</div>" +
        '<div style="margin-bottom:4px;">' +
        UI.chk("mcp-ed-allowed-ro", srv.allowed_tools_read_only || false, "Read-only filter") +
        "</div>" +
        '<div style="margin-bottom:8px;">' +
        UI.lbl("Require Approval") +
        UI.sel(
          "mcp-ed-approval",
          '<option value="always"' +
            (srv.require_approval === "always" ? " selected" : "") +
            ">Always</option>" +
            '<option value="never"' +
            (srv.require_approval === "never" ? " selected" : "") +
            ">Never</option>" +
            '<option value="filter"' +
            (srv.require_approval === "filter" ? " selected" : "") +
            ">Filter</option>",
        ) +
        "</div>" +
        '<div id="mcp-ed-filter-ui" style="' +
        (srv.require_approval === "filter" ? "" : "display:none;") +
        'margin-bottom:8px;padding-left:12px;border-left:2px solid var(--m-border,#ddd);">' +
        '<div style="font-size:12px;font-weight:bold;margin:6px 0;color:var(--m-text,#222);">Always:</div>' +
        UI.lbl("Tools") +
        UI.inp(
          "mcp-ed-appr-tools",
          "text",
          srv.require_approval_tools || "",
          ' style="width:100%;"',
        ) +
        '<div style="margin:4px 0;">' +
        UI.chk("mcp-ed-appr-ro", srv.require_approval_read_only || false, "Read-only") +
        "</div>" +
        '<div style="font-size:12px;font-weight:bold;margin:8px 0 6px;color:var(--m-text,#222);">Never:</div>' +
        UI.lbl("Tools") +
        UI.inp(
          "mcp-ed-appr-never-tools",
          "text",
          srv.require_approval_never_tools || "",
          ' style="width:100%;"',
        ) +
        '<div style="margin:4px 0;">' +
        UI.chk("mcp-ed-appr-never-ro", srv.require_approval_never_read_only || false, "Read-only") +
        "</div>" +
        "</div>" +
        '<div style="margin-bottom:8px;">' +
        UI.chk("mcp-ed-defer", srv.defer_loading || false, "Defer Loading") +
        "</div>" +
        '<div style="display:flex;gap:8px;margin-top:10px;">' +
        UI.btn("mcp-ed-save", isNew ? "Создать" : "Сохранить", "accent") +
        UI.btn("mcp-ed-cancel", "Отмена") +
        "</div>";
      ...
      document.getElementById("mcp-ed-approval").addEventListener("change", function () {
        document.getElementById("mcp-ed-filter-ui").style.display =
          this.value === "filter" ? "" : "none";
      });

      document.getElementById("mcp-ed-cancel").addEventListener("click", function () {
        editorEl.remove();
        editorEl = null;
      });
      ...
      document.getElementById("mcp-ed-save").addEventListener("click", function () {
        var payload = {
          ...
          require_approval_never_tools: document
            .getElementById("mcp-ed-appr-never-tools")
            .value.trim(),
        ...
        window.AliceDispatcher.request(url, {
          method: method,
    ...
    document.getElementById("mcp-mgr-add").addEventListener("click", function () {
      openMcpEditor(null);
    });
    refreshList();
  };
