...
def _row_to_dict(row: Any) -> dict[str, Any]:
    return {
        ...
        "description": row["description"], "agent_id": row["agent_id"],
        ...
        "status": row["status"], "metadata": _parse(row["metadata_json"], {}),
...
def upsert_department(data: Mapping[str, Any], department_id: Optional[str] = None) -> dict[str, Any]:
    init_department_tables()
    ...
    conn = get_conn()
    try:
        existing = conn.execute("SELECT created_at FROM departments WHERE id = ?", (ident,)).fetchone()
        ...
        conn.execute(
            ...
            (ident,name,str(data.get("type") or "general"),payload["description"],payload["agent_id"],
             _json(payload["model_config"]),payload["system_prompt"],_json(payload["tools"]),
             _json(payload["capabilities"]),_json(payload["policies"]),_json(payload["knowledge_scope"]),
             _json(payload["session_config"]),payload["status"],_json(payload["metadata"]),created_at,now),
        )
        conn.commit()
    finally:
        conn.close()
...
def departments_update(department_id: str):
    if not _admin_ok():
        return jsonify({"error":"department admin authorization required"}), 401
    try:
        return jsonify({"department":upsert_department(request.get_json(silent=True) or {}, department_id)})
    except ValueError as exc:
        return jsonify({"error":str(exc)}), 400
...
def departments_delete(department_id: str):
    ...
    conn = get_conn()
    try:
        conn.execute("UPDATE departments SET status='archived',updated_at=? WHERE id=?", (_now(),department_id))
        conn.commit()
    finally:
        conn.close()
