...
YANDEX_OBJECT_STORAGE_STANDARD_RU = ProviderStorageTariff(
    ...
    region="ru",
    storage_per_gb_month=2.376,
    free_storage_gb_month=1.0,
    get_per_10k=0.46,
    free_get_per_month=100_000,
    write_per_10k=None,
    free_write_per_month=10_000,
)
...
def calculate_provider_storage_cost(
    *,
    bytes_stored: int,
    lifetime_days: float,
    tariff: ProviderStorageTariff,
    ...
    write_cost = (
        None if tariff.write_per_10k is None else billable_writes / 10_000 * tariff.write_per_10k
    )
    components = {
        "storage": round(storage_cost, 6),
        "get_operations": None if get_cost is None else round(get_cost, 6),
    ...
    return {
        ...
        "pricing_version": tariff.pricing_version,
        "pricing_source": tariff.source,
        "pricing_source_updated": tariff.source_updated,
    ...
    *,
    ...
    components = {
        ...
        "vector_storage": _cost(
            config.vector_storage_per_mb_day, vector_bytes / (1024 * 1024) * max(0.0, lifetime_days)
        ),
        "metadata": _cost(
            config.metadata_per_kb_day, metadata_bytes / 1024 * max(0.0, lifetime_days)
        ),
        "backup": _cost(
            config.backup_per_mb_day, backup_bytes / (1024 * 1024) * max(0.0, lifetime_days)
        ),
    ...
    def evaluate(
        ...
        *,
        ...
        if value >= self._policy.reward_value_threshold and cost == 0:
            ...
            outcome, amount, default_reason = (
                "penalty",
                min(cost, self._policy.max_penalty),
                "cost/value policy threshold",
            )
        else:
