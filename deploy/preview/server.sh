...
cleanup_key() {
  local key="$1"
  [[ "$key" =~ ^[a-z0-9-]{1,50}$ ]] || die "invalid cleanup key: $key"
...
deploy() {
  ...
  [[ "$ttl" =~ ^[0-9]+$ ]] || die "invalid TTL: $ttl"
  (( ttl > 0 && ttl <= 720 )) || die "TTL must be between 1 and 720 hours"
...
cleanup_expired() {
  ...
  docker ps -aq --filter "label=alice.preview=true" | while read -r container_id; do
    ...
    if [[ "$expires" =~ ^[0-9]+$ ]] && (( expires <= now )) && [[ "$key" =~ ^[a-z0-9-]{1,50}$ ]]; then
      cleanup_key "$key"
    fi
  done
}
...
case "$ACTION" in
  deploy)
    [[ $# -eq 5 ]] || die "usage: server.sh deploy <key> <base_path> <archive> <ttl_hours>"
    deploy "$KEY" "$BASE_PATH" "$ARCHIVE" "$TTL_HOURS"
    ;;
  cleanup)
    [[ $# -eq 2 ]] || die "usage: server.sh cleanup <key>"
    require_key
    cleanup_key "$KEY"
    ;;
  cleanup-expired)
    [[ $# -eq 1 ]] || die "usage: server.sh cleanup-expired"
    cleanup_expired
    ;;
  *)
