...
create table if not exists public.budget_operations (
...
create index if not exists budget_operations_budget_created_idx
  on public.budget_operations (budget_id, created_at desc);

create or replace function public.apply_budget_operation(
  p_budget_id text,
  p_account_type text,
...
as $$
declare
  a public.budget_accounts%rowtype;
  result jsonb;
begin
  if p_amount < 0 then raise exception 'amount must be non-negative'; end if;

  select * into a from public.budget_accounts
  ...
  if p_idempotency_key is not null then
    select jsonb_build_object('operation_id', operation_id, 'status', status)
      into result from public.budget_operations
  ...
  if p_operation_type = 'RESERVE' then
    ...
    update public.budget_accounts set reserved = reserved + p_amount,
    ...
    update public.budget_accounts set spent = spent + p_amount,
      loss_today = case when loss_period = current_date then loss_today + p_amount else p_amount end,
      loss_period = current_date, version = version + 1, updated_at = now()
      where budget_id = p_budget_id and account_type = p_account_type;
    ...
    update public.budget_accounts set won = won + p_amount,
      version = version + 1, updated_at = now()
      where budget_id = p_budget_id and account_type = p_account_type;
  else
...
revoke all on function public.apply_budget_operation(text,text,text,numeric,text,text) from public;
