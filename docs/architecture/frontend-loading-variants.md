...
- `single` inlines locally served CSS and JavaScript into the same HTML
  ...
  default.
- `packed` embeds the single-file HTML as a gzip/base64 payload and uses the
  browser `DecompressionStream` API to restore it. A visible fallback remains
  available if JavaScript or decompression fails.
