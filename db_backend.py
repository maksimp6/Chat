"""Database backend selection and a small SQLite-compatible PostgreSQL adapter.

Production uses PostgreSQL through ALICE_DATABASE_URL/DATABASE_URL. Tests and local
legacy tooling can continue to use SQLite when those variables are absent.
"""
...
try:
    import psycopg
except ImportError:  # pragma: no cover - exercised only when PostgreSQL is selected
    psycopg = None
...
class PGRow:
    """sqlite3.Row-like mapping with both name and numeric indexing."""
...
def translate_sql(sql: str) -> str:
    """Translate the small SQLite SQL subset used by Alice Pro to PostgreSQL."""
    ...
    # Psycopg treats literal `%` as placeholder syntax. Escape percent signs\n    # that are not parameter placeholders before translating SQLite `?`.\n    text = re.sub(r"%(?![sbt%])", "%%", text)\n    return _QMARK_RE.sub("%s", text)\n

class PGCursor:
    def __init__(self, connection: "PGConnection", raw_cursor) -> None:
    ...
    def execute(self, sql: str, params: Iterable[Any] = ()) -> "PGCursor":
        ...
        try:
            ...
            if name in _UNIQUE_VIOLATION_NAMES:
...
class PGConnection:
    ...
    def cursor(self) -> PGCursor:
        return PGCursor(self, self._raw.cursor())
