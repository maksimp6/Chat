"""Autonomous MCP servers storage, backed by Alice Pro's selected database backend."""
import os
import uuid
import json
import logging
    ...
    try:
        ...
        conn.execute(
            ...
            (
                ...
                data["headers"],
                data["allowed_tools"],
                data["allowed_tools_read_only"],
                data["require_approval"],
                data["require_approval_tools"],
                data["require_approval_read_only"],
                data["require_approval_never_tools"],
                data["require_approval_never_read_only"],
                data["defer_loading"],
...
def get_conv_mcp_settings(conv_id):
    ...
    try:
        row = conn.execute(
            "SELECT * FROM conv_mcp_settings WHERE conversation_id=?", (conv_id,)
        ).fetchone()
        if row:
            return {
...
def get_enabled_servers_for_conv(conv_id):
    ...
    try:
        row = conn.execute(
            "SELECT enabled_mcp_servers FROM conv_mcp_settings WHERE conversation_id=?",
            (conv_id,),
        ).fetchone()
        if not row:
            return []
...
def get_server_config(server_id: str) -> dict:
    ...
    try:
        row = conn.execute(
            "SELECT connector_id, config FROM mcp_servers WHERE id = ?", (server_id,)
        ).fetchone()
        if not row:
            return {}
