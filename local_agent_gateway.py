...
def wait_for_local_tool_job(job_id: str, *, timeout_seconds: float = 30.0, poll_interval: float = 0.25):
    """Wait for a queued remote tool job to finish without exposing a public waiter route."""
    deadline = _now() + max(0.1, float(timeout_seconds))
    delay = max(0.05, float(poll_interval))
    while _now() < deadline:
        conn = get_conn()
        try:
            row = conn.execute(
                "SELECT status, result_json, error_json FROM local_agent_jobs WHERE id = ?",
                (job_id,),
            ).fetchone()
        finally:
            conn.close()
