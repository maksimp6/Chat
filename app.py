...
def patch_conversation(conv_id):
    ...
    if "title" in data: update_conversation_title(conv_id, data["title"])
    if "model" in data: update_conversation_model(conv_id, data["model"])
...
def api_memory_panel_data():
    cfg = load_memory_config(); conn = get_conn(); conn.row_factory = sqlite3.Row
    facts = [dict(r) for r in conn.execute("SELECT * FROM global_memory ORDER BY updated_at DESC").fetchall()]
    conn.close(); return jsonify({"config": cfg, "facts": facts})

@app.route("/api/memory/config", methods=["PUT"])
def api_update_memory_config():
    data = request.get_json(silent=True) or {}; save_memory_config(data)
...
def treasury_top_up():
    data = request.get_json(silent=True) or {}
    try:
        return jsonify({"demo": True, "account": demo_top_up("default", data.get("amount"), data.get("description", "Demo top-up"))}), 201
