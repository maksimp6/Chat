...
def _clean_mcp_tool(tool):
    ...
    if tool.get("type") != "mcp":
        return tool
    # Локальные коннекторы не должны отправляться как облачные MCP
    ...
    if not url and not cid.startswith("connector_"):
        return None  # Отсекаем пустые server_url
...
class YandexResponsesClient(YandexFileManagerMixin):
    ...
    def _resolve_yandex_conv_id(self, conv_id):
        ...
        # Если ID не UUID (например conv_1789132886), используем связку через SQLite
        ...
        try:
            ...
            # Создаем реальный диалог в облаке Yandex
    ...
    def ask(self, message, model_key, conversation_id=None, params=None):
        ...
        if params.get("tools"):
            ...
            api_logger.info(f"[ASK] Tools cleaned: {len(payload['tools'])} tools, MCP stripped of transport/valid/null")
...
def _build_function_tools(connectors):
    ...
    if 'local_git' in connectors:
        for name, config in GIT_TOOLS.items():
            tools.append({
                "type": "function",
                "name": name,
    ...
    if 'termux_api' in connectors:
        for name, config in TERMUX_TOOLS.items():
            tools.append({
                "type": "function",
                "name": name,
                "description": config["description"],
...
class YandexMcpMixin:
    ...
    def ask_with_mcp(self, message, model_key, conversation_id=None, params=None):
        ...
        # Стратегия 1: Отправка function_call_output в Responses API
        try:
            fc_inputs = [
                {
            ...
            final_response = self.ask(
                ...
                model_key,
                conversation_id,
                {**final_params, "input": fc_inputs}
            )
            text = self.extract_text(final_response)
            if text:
                return final_response
        except Exception as e1:
            api_logger.info(f"[MCP] Стратегия 1 (function_call_output) не сработала: {e1}")

        # Стратегия 2: Отправка role: tool без привязки к conversation
        try:
            tool_msg_inputs = [
                {
                    "role": "tool",
            ...
            final_response = self.ask(
                ...
                model_key,
                None,
                {**final_params, "input": new_input}
            )
            text = self.extract_text(final_response)
            if text:
                return final_response
        except Exception as e2:
            api_logger.info(f"[MCP] Стратегия 2 (role: tool) не сработала: {e2}")

        # Стратегия 3: Генерация финального объяснения по контексту вывода
        try:
            ...
            return {
                "output_text": "Команда выполнена успешно:\n\n" + "\n".join(raw_outputs_text),
                "status": "completed"
            }
