...
class ElementShim extends EventTargetShim {
  ...
  get innerHTML() {
    return this.children.map((child) => serialize(child)).join("") || this._text;
  }
  set innerHTML(value) {
    this.children = [];
    this._text = "";
  ...
  querySelectorAll(selector) {
    ...
    for (const part of parts) {
      ...
      for (const root of current) {
        walk(root, (node) => {
          if (node !== root && matches(node, part)) next.push(node);
        });
      }
      current = unique(next);
    }
...
class DocumentShim extends ElementShim {
  ...
  get documentElement() {
    return this.body.querySelector("html") || this.body;
  }
}
...
function parseHTML(html, root) {
  ...
  while ((token = tokenRe.exec(html))) {
    ...
    const element = new ElementShim(
      nameMatch[1],
      parseAttributes(raw.slice(nameMatch[0].length, -1)),
    );
...
function serialize(element) {
  ...
  return (
    ...
    ">" +
    element._text +
    element.children.map(serialize).join("") +
    "</" +
    element.tagName.toLowerCase() +
    ">"
  );
...
function assertLayoutRule(declarations, property, expected) {
  if (declarations[property] !== expected) {
    throw new Error(
      "Expected " + property + ": " + expected + ", got " + (declarations[property] || "<missing>"),
    );
  }
}
