...
def test_anonymous_registration_is_idempotent_and_sanitizes_metadata():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            init_user_identity_table()
            first = register_anonymous_user(
                "android-installation-123456", {"platform": "android", "token": "must-not-persist"}
            )
            second = register_anonymous_user(
                "android-installation-123456", {"platform": "android-16"}
            )
