...
(async function main() {
    {
        const {document, window} = load(
            ["static/dispatcher.js", "static/core.js"],
...

    {
        const {document, context} = load(
            ["static/dispatcher.js", "static/core.js", "static/models.js"],
        ...
        vm.runInContext(`modelsData = {text: {lite: {name: "Lite", input: "1", output: "2", multimodal: true}, full: {name: "Full", input: "3", output: "4"}}, voice: {voice: {name: "Voice", input: "5", output: "6"}}}; currentModel = "lite"; window.changeModel = function(model) { currentModel = model; };`, context);
    ...
    {
        const {document, context} = load(
            ["static/dispatcher.js", "static/core.js", "static/sidebar.js"],
...

    {
        ...
        const {document, window} = load(
            ["static/dispatcher.js", "static/memory_panel.js", "static/header_actions.js"],
            '<button id="memory-btn"></button><div id="memoryModal" hidden><button id="memoryCloseBtn"></button><button id="memoryClearBtn"></button><input id="memEnabled"><input id="memLimit"><span id="memCount"></span><div id="memoryFactsList"></div>',
            {fetch}
        );
        ...
        assert.deepEqual(requests.map((item) => item.url), ["/api/memory/manage"]);
