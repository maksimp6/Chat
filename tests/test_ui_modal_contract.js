...
if (modal.getAttribute("aria-hidden") !== "true")
  throw new Error("hidden modal must expose aria-hidden=true");
if (modal.getAttribute("aria-labelledby") !== "contract-modal-title")
  throw new Error("modal must be labelled by its title");

const content = modal.querySelector(".modal-content");
if (!content) throw new Error("modal requires one .modal-content shell");
if (modal.querySelectorAll(".modal-content").length !== 1)
  throw new Error("modal requires exactly one content shell");

const close = modal.querySelector(".modal-close");
if (!close || !close.classList.contains("alice-btn"))
  throw new Error("modal close must use button contract");
if (close.dataset.action !== "contract.modal.close")
  throw new Error("close button must use action contract");

UI.modal.open(modal);
if (modal.hidden || !modal.classList.contains("visible")) throw new Error("open must expose modal");
if (modal.getAttribute("aria-hidden") !== "false")
  throw new Error("open modal must expose aria-hidden=false");

UI.modal.close(modal);
if (!modal.hidden || modal.classList.contains("visible")) throw new Error("close must hide modal");
if (modal.getAttribute("aria-hidden") !== "true")
  throw new Error("closed modal must expose aria-hidden=true");
...
UI.modal.open(modal);
close.click();
if (closeCalls !== 1 || !modal.hidden)
  throw new Error("close click must dispatch exactly once and close modal");
...
if (dispatcherModal.hidden)
  throw new Error("modal.open must resolve data-modal through dispatcher params");

dispatcherClose.click();
if (!dispatcherModal.hidden)
  throw new Error("modal.close must resolve data-modal through dispatcher params");
...
if (!dispatcherModal.hidden)
  throw new Error("failed modal action must not corrupt another modal state");

brokenOpen.dataset.modal = "dispatcher-modal";
brokenOpen.click();
console.error = originalError;
if (dispatcherModal.hidden)
  throw new Error("same button must remain usable after a failed modal action");

dispatcherClose.click();
if (!dispatcherModal.hidden)
  throw new Error("modal must still close after recovery from action failure");
