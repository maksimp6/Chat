...
class TestExecutionTraceResponses(unittest.TestCase):
    ...
    def test_lifecycle_events_are_preserved_but_trace_finalized_is_internal(self):
        ...
        self.assertEqual(
            types, ["api_poll_completed", "api_request_completed", "api_response_received"]
        )
    ...
    def test_circular_trace_reference_is_not_serialized(self):
        ...
        data = trace.finalize()
        self.assertEqual(
            data["request"]["execution_trace"], {"trace_id": trace.trace_id, "<circular_ref>": True}
        )
    ...
    def test_two_ai_steps_aggregate_without_double_counting(self):
        ...
        self.assertAlmostEqual(
            billing["total_cost"], sum(i["total_cost"] for i in billing["items"]), places=6
        )
...
class TestYandexResponsesPollingTrace(unittest.TestCase):
    ...
    def test_poll_updates_existing_initial_response(self):
        ...
        trace = ExecutionTrace()
        trace.add_response(
            {"id": "resp-1", "status": "in_progress", "output": []},
            step_index=1,
            kind="initial_response",
        )
        client.session.get.side_effect = [
            self._response({"id": "resp-1", "status": "completed", "output": [{"type": "message"}]})
        ]
    ...
    def test_ask_passes_prompt_cache_key(self):
        ...
        with patch(
            "yandex_client_modules.request_mixin._resolve_global_provider_credential"
