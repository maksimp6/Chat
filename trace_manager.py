...
class ExecutionTrace:
    ...
    def bind_context(self, context: Any) -> None:
        """Bind immutable InvocationContext identifiers to this trace."""
        self.trace.update({
    ...
    def get_metadata(self) -> Dict[str, str]:
        return {
            ...
            **({"session_id": str(self.trace["session_id"])} if self.trace.get("session_id") else {}),
            **({"conversation_id": str(self.trace["conversation_id"])} if self.trace.get("conversation_id") else {}),
            **({"invocation_id": str(self.trace["invocation_id"])} if self.trace.get("invocation_id") else {}),
        }
    ...
    def add_response(self, raw_json: Dict[str, Any], step_index: int = 1,
                     start_timestamp: Optional[float] = None, end_timestamp: Optional[float] = None,
                     timing_ms: Optional[float] = None, **kwargs) -> None:
        idx = step_index or kwargs.get("call_index", 1)
        clean_raw = ({k: v for k, v in raw_json.items() if k not in ("trace", "step_timings", "execution_trace")}
        ...
        response_entry = {
            "step": idx, "timestamp": end_timestamp, "raw": clean_raw,
